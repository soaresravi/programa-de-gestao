    ...
    return (
        <div className={styles.calendarioContainer}>
          <div className={styles.calendarioHeader}>
            <button onClick={mesAnterior} className={styles.mesButton}>
              <ChevronLeft size={20} strokeWidth={2.5} />
            </button>
            <span className={styles.mesTitulo}>{formatarMes(currentMonth)}</span>
            <button onClick={proximoMes} className={styles.mesButton}>
          ...
          <div className={styles.diasGrid}>
            {dias.map(({ date, isCurrentMonth }, index) => {
              ...
              return (
                <button
                  key={index}
                  className={`${styles.diaButton} ${!isCurrentMonth ? styles.otherMonth : ''} ${isInRangeValue ? styles.inRange : ''} ${isStart ? styles.startDate : ''} ${isEnd ? styles.endDate : ''}`}
